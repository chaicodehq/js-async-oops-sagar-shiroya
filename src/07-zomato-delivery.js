...
export function confirmOrder(order) {
    return new Promise((resolve, reject) => {
        if (order.status !== "placed" || !("orderId" in order)) {
...
export function assignRider(order) {
    ...
    return new Promise((resolve, reject) => {
        ...
        resolve({
            ...order,
            rider: riderPool[Math.floor(Math.random() * riderPool.length)],
            status: "assigned",
        });
    });
}
...
export function deliverOrder(order) {
    return new Promise((resolve, reject) => {
        if (order.status !== "assigned" || !("rider" in order)) {
...
export function processMultipleOrders(orderList) {
    const results = orderList.map((order) =>
        processDelivery(order.restaurant, order.items).then((res) => {
